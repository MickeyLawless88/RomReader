...
/* ========================================================================
   DIALOG BOX ENGINE
   All dialogs are drawn centered on screen over the hex editor.
   The editor is redrawn behind them (already on screen) and the dialog
   is painted on top using direct cprintf/gotoxy calls, then erased after.
   ======================================================================== */


/* Draw a single dialog box frame. Returns screen row of first content line. */
int DlgDraw(int w, int h, const char *title) {
    ...
    buf[0] = '\xC9'; /* double-line TL corner */
    for (i = 1; i < w - 1; i++) buf[i] = '\xCD'; /* double horiz */
    buf[w - 1] = '\xBB'; /* TR corner */
    buf[w] = '\0';
    cprintf("%s", buf);
    ...
    return startY + 3; /* first usable content row */
}

/* Print a centered line inside dialog at given screen row */
void DlgLine(int screenRow, int dlgW, const char *text) {
...
/* Print a left-aligned line inside dialog */
void DlgLineLeft(int screenRow, int dlgW, const char *text) {
...
/* Draw a highlighted menu item inside dialog */
void DlgMenuItem(int screenRow, int dlgW, const char *text, int selected) {
    int startX = (SCREEN_W - dlgW) / 2 + 2;
    int inner = dlgW - 4;
    int i;
...
/* Draw an input field inside dialog. buf is the current content. */
...
/* Read a string inside a dialog input field. Returns 0 on ESC, 1 on ENTER. */
...
/* Show a simple message dialog with OK (any key) */
...
/* Show a menu dialog. items[] is null-terminated array of strings.
   Returns 1-based selection, or 0 on ESC. */
int DlgMenu(const char *title, const char **items, int count) {
    int w = 54;
    int h = count + 6;
    int r = DlgDraw(w, h, title);
...
void DisplayHexEditor(void) {
    ...
    unsigned int addr;
    unsigned char byte;
    int maxLines = 19;  /* rows 1=title 2=header 3=sep 4..22=data 23=help 24=status */
    const char *modeStr;
    int cursorCol;
...
void UploadToEPROM(void) {
    ...
    /* ---- Start address dialog for BYTEK ---- */
    addr = 0;
    if (fmt == 2) {
        char abuf[16];
        int w = 54, h = 9, r;
    ...
    /* ================================================================
       FORMAT 1: Standard Binary - raw bytes from DataBuffer
       ================================================================ */
    if (fmt == 1) {
    ...
    /* ================================================================
       FORMAT 2: BYTEK Binary Code 20
       Header: 2-byte count (H/L), 3-byte address (H/M/L)
       Data: raw bytes
       Footer: 16-bit sumcheck (H/L, carries from bit 15 dropped), 2x 0x00
       ================================================================ */
    else if (fmt == 2) {
        unsigned long sent = 0;
        unsigned int sc = 0;
    ...
    /* ================================================================
       FORMAT 3: DATA I/O Binary Code 10
       Arrow header (5 bytes: 0x01 0x00 0x00 0x00 0x17)
       1 NULL (0x00)
       4-nibble HEX byte count (ASCII hex digits of count)
       1 rubout / start code (0xFF)
       Binary data
       2 NULLs (0x00 0x00)
       2-byte HEX sumcheck (ASCII hex of 16-bit sum)
       ================================================================ */
    else if (fmt == 3) {
        unsigned long sent = 0;
        unsigned int sc = 0;
        ...
        /* NULL */
        WriteCOM1(0x00);
        /* 4-nibble HEX byte count as ASCII */
    ...
    /* ================================================================
       FORMAT 4: DEC Binary Code 11
       32 rubouts (0xFF) as leader
       1 NULL (0x00)
       Raw binary data
       ================================================================ */
    else if (fmt == 4) {
    ...
    /* ================================================================
       FORMAT 5: Motorola S-Record S1
       S0 header record
       S1 data records (16 bytes each): S1LLAAAADDDD...CC
         LL = byte count (address + data + checksum)
         AAAA = 16-bit address
         DD... = data bytes
         CC = one's complement of sum of LL+addr+data bytes
       S9 end record
       ================================================================ */
    else if (fmt == 5) {
        unsigned long laddr = 0;
        ...
        /* S0 header */
        {
            const char *hdr = "HDR";
            unsigned int hcs = 0;
            unsigned int hlen = (unsigned int)(strlen(hdr) + 3); /* len+addr(2)+data+cs */
            hcs = hlen + 0x00 + 0x00;
        ...
        while (laddr < byteCount) {
            ...
            cs = (recLen + 3);  /* +3 for addr hi, addr lo, checksum byte itself */
