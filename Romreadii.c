...
#define MAX_BUFFER_SIZE 65536U  /* 64KB buffer - requires farmalloc */
...
void DisplayHexEditor(void) {
    ...
    cprintf(" EPROM Editor v1.2 - Mickey W. Lawless [%s MODE]%s", modeStr, Modified ? " *MOD*" : "     ");
    cprintf("%-21s", "");
...
int main(int argc, char *argv[]) {
    ...
    if (DataBuffer == NULL) {
        ...
        printf("Cannot allocate %u byte buffer.\n", MAX_BUFFER_SIZE);
        textcolor(LIGHTGRAY);
    ...
    if (argc > 1) {
        ...
        textcolor(LIGHTCYAN);
        printf("========================================================================\n");
        printf("               EPROM Data Reader & Hex Editor v1.2\n");
...
void LoadFileIntoBuffer(void) {
    ...
    while (BufferSize < MAX_BUFFER_SIZE && fread(&byte, 1, 1, fp) == 1) {
        ...
        if ((BufferSize % 256) == 0) printf("\rBytes loaded: %u", BufferSize);
    }
