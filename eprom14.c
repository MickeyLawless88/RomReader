 ...
 *
 * Change log:
 *   v1.4 - FIX: InitCOM1() FCR trigger level changed from 0x07 (14-byte
 *          threshold) to 0x01 (1-byte threshold).  With 0x07 the LSR
 *          data-ready bit never asserted unless 14 bytes accumulated,
 *          so COM1DataReady() always returned 0 and the buffer stayed
 *          empty.
 *        - FIX: InitCOM1() drain loop replaced with non-blocking version.
 *          Old code called ReadCOM1() which blocks forever if a framing/
 *          parity error held the LSR data-ready bit high with no actual
 *          byte waiting.
 *        - FIX: Removed getch() wait in main() between SendInitString()
 *          and ReadDataFromCOM1().  The programmer responds to the init
 *          string immediately; the blocking wait caused the 16-byte UART
 *          FIFO to overflow and all incoming data was lost before the
 *          read loop started.
 *        - FIX: Timeout threshold raised from expectedSize/2 to 95% of
 *          expectedSize so a brief mid-transfer pause no longer triggers
 *          an early exit after only half the ROM is received.
...
/* Dialog engine forward declarations */
int  DlgDraw(int w, int h, const char *title);
void DlgLine(int screenRow, int dlgW, const char *text);
void DlgLineLeft(int screenRow, int dlgW, const char *text);
void DlgMenuItem(int screenRow, int dlgW, const char *text, int selected);
void DlgInputField(int screenRow, int dlgW, const char *label, const char *buf, int cursorVisible);
int  DlgInput(int screenRow, int dlgW, const char *label, char *buf, int maxLen);
void DlgMessage(const char *title, const char *line1, const char *line2, int isError);
int  DlgYesNo(const char *title, const char *question);
int  DlgMenu(const char *title, const char **items, int count);
...
/* ========================================================================
   InitCOM1 - v1.4 patched
   Changes from v1.3:
     - FCR written twice: first 0x07 to clear both FIFOs, then 0x01 to
       enable FIFO with a 1-byte trigger level.  The old single write of
       0x07 left the trigger at 14 bytes, so COM1DataReady() (which tests
       LSR bit 0) never asserted until 14 bytes had accumulated.  On a
       slow or short transfer this meant the buffer stayed empty.
     - Drain loop is now non-blocking: reads inportb(COM1_DATA) directly
       instead of calling ReadCOM1() which spins forever if a line-status
       error bit holds LSR bit 0 high with no real byte present.
   ======================================================================== */
void InitCOM1(unsigned int baud) {
    ...
    outportb(COM1_IER, 0x00);                    /* disable all interrupts        */
    outportb(COM1_LCR, 0x80);                    /* DLAB=1: access baud divisor   */
    outportb(COM1_DATA, divisor & 0xFF);          /* divisor low byte  (0x3F8)     */
    outportb(COM1_IER,  (divisor >> 8) & 0xFF);  /* divisor high byte (0x3F9)     */
    outportb(COM1_LCR, 0x03);                    /* DLAB=0, 8N1                   */

    /* FCR (at 0x3FA, same address as read-only IIR):
       First write clears both FIFOs (bits 1+2), second write enables
       FIFO with a 1-byte trigger level (bit 0 only).
       Trigger level 0x07 = 14 bytes on a 16550 - that was the old bug. */
    outportb(COM1_IIR, 0x07);   /* clear RX + TX FIFOs            */
    delay(5);
    outportb(COM1_IIR, 0x01);   /* enable FIFO, 1-byte trigger     */
    delay(5);

    outportb(COM1_MCR, 0x0B);   /* DTR + RTS + OUT2                */
    delay(20);                  /* let line settle                  */

    /* Drain any garbage already in the RX FIFO.
       Non-blocking: read inportb() directly, never call ReadCOM1()
       which would block forever if LSR is stuck with an error bit. */
    for (i = 0; i < 200; i++) {
        if (inportb(COM1_LSR) & 0x01)
            inportb(COM1_DATA);  /* discard byte */
        else
            delay(1);
    }
}
    ...
    */
    ...
    while (BufferSize < MAX_BUFFER_SIZE) {
        ...
        if (!(inportb(COM1_LSR) & 0x01)) {
            ...
            /* Timeout only fires after nearly all expected bytes received.
               v1.4: threshold raised from expectedSize/2 to 95% so a brief
               mid-transfer pause no longer cuts the read short at the halfway
               mark.  When expectedSize==0 (unknown) keep the old 64-byte
               threshold so the waiting message still appears promptly. */
            {
                unsigned long threshold;
                if (expectedSize > 0)
                    threshold = (expectedSize * 95UL) / 100UL;
                else
                    threshold = 64UL;
...
void SaveBufferToFile(void) {
    ...
    filename[0] = '\0';
    r = DlgDraw(w, h, " Save Binary File ");
    DlgLine(r,     w, "Enter filename to save:");
    DlgLine(r + 1, w, "(ESC to cancel)");
    if (!DlgInput(r + 3, w, "File:", filename, 12)) {
        DisplayHexEditor(); return;
    }
    if (filename[0] == '\0') { DisplayHexEditor(); return; }

    fp = fopen(filename, "wb");
    if (fp == NULL) {
        DlgMessage(" Error ", "Cannot create file!", filename, 1);
        return;
    }
    ...
    {
        char msg[50];
        sprintf(msg, "Saved %lu bytes OK", BufferSize);
        DlgMessage(" Saved ", msg, filename, 0);
    }
    Modified = 0;
}
...
void SaveBufferAsHex(void) {
...

    {
        int w = 54, h = 9, r;
        filename[0] = '\0';
        r = DlgDraw(w, h, " Save Intel HEX File ");
        DlgLine(r,     w, "Enter .hex filename:");
        DlgLine(r + 1, w, "(ESC to cancel)");
        if (!DlgInput(r + 3, w, "File:", filename, 12)) {
            DisplayHexEditor(); return;
        }
    ...
    {
        char msg[50];
        sprintf(msg, "Intel HEX saved: %lu bytes", BufferSize);
        DlgMessage(" Saved ", msg, filename, 0);
    }
}
...
void SaveListing(void) {
...

    {
        int w = 54, h = 9, r;
        filename[0] = '\0';
        r = DlgDraw(w, h, " Save Hex Listing ");
        DlgLine(r,     w, "Enter listing filename:");
        DlgLine(r + 1, w, "(ESC to cancel)");
        if (!DlgInput(r + 3, w, "File:", filename, 12)) {
            DisplayHexEditor(); return;
        }
...
void PrintBuffer(void) {
    ...
    if (!DlgYesNo(" Print ", "Print buffer to LPT1?"))
        return;

    {
        int w = 44, h = 6;
        DlgDraw(w, h, " Printing ");
        DlgLine((SCREEN_H - h) / 2 + 3, w, "Sending to LPT1... ESC=cancel");
    }

    fp = fopen("LPT1", "w");
    if (fp == NULL) {
        DlgMessage(" Error ", "Cannot access LPT1!", "Check printer.", 1);
        return;
    }
    ...
    for (addr = 0; addr < BufferSize; addr += BYTES_PER_LINE) {
        if (kbhit() && getch() == 27) {
            fclose(fp);
            DlgMessage(" Cancelled ", "Printing cancelled.", "", 0);
            return;
        }
        fprintf(fp, "%04X:    ", addr);
        for (col = 0; col < BYTES_PER_LINE && addr + col < BufferSize; col++) {
    ...
    fprintf(fp, "\f");
    fclose(fp);
    DlgMessage(" Done ", "Printing complete!", "", 0);
}
...
int DlgDraw(int w, int h, const char *title) {
    int x, y, i, startX, startY;
    char buf[82];

    if (w > DLG_MAX_W) w = DLG_MAX_W;
    if (h > DLG_MAX_H) h = DLG_MAX_H;

    startX = (SCREEN_W - w) / 2 + 1;
    startY = (SCREEN_H - h) / 2;
    ...
    gotoxy(startX, startY + 1);
    {
        ...
        int pad = (w - 2 - tlen) / 2;
        int rpad = w - 2 - tlen - pad;
        cprintf("\xBA");
        textbackground(CYAN); textcolor(BLACK);
        for (i = 0; i < pad; i++) cprintf(" ");
        cprintf("%s", title);
        for (i = 0; i < rpad; i++) cprintf(" ");
    ...
    buf[0] = '\xC7';
    for (i = 1; i < w - 1; i++) buf[i] = '\xC4';
    buf[w - 1] = '\xB6';
    buf[w] = '\0';
    cprintf("%s", buf);

    for (y = startY + 3; y < startY + h - 1; y++) {
        gotoxy(startX, y);
        cprintf("\xBA");
        for (i = 0; i < w - 2; i++) cprintf(" ");
        cprintf("\xBA");
    }

    gotoxy(startX, startY + h - 1);
    buf[0] = '\xC8';
    for (i = 1; i < w - 1; i++) buf[i] = '\xCD';
    buf[w - 1] = '\xBC';
    buf[w] = '\0';
...
void DlgLine(int screenRow, int dlgW, const char *text) {
    int startX = (SCREEN_W - dlgW) / 2 + 1;
    int tlen = strlen(text);
    int col = startX + 1 + (dlgW - 2 - tlen) / 2;
    textcolor(WHITE);
    textbackground(BLUE);
    gotoxy(col, screenRow);
    cprintf("%s", text);
    textcolor(LIGHTGRAY);
    textbackground(BLACK);
}
...
void DlgLineLeft(int screenRow, int dlgW, const char *text) {
    int startX = (SCREEN_W - dlgW) / 2 + 2;
    textcolor(WHITE);
    textbackground(BLUE);
    gotoxy(startX, screenRow);
    cprintf("%s", text);
    textcolor(LIGHTGRAY);
    textbackground(BLACK);
}
...
void DlgMenuItem(int screenRow, int dlgW, const char *text, int selected) {
    ...
    int inner = dlgW - 4;
    int i;
    if (selected) { textcolor(BLACK); textbackground(LIGHTCYAN); }
    else          { textcolor(YELLOW); textbackground(BLUE); }
    gotoxy(startX, screenRow);
    cprintf(" %s", text);
    for (i = strlen(text) + 1; i < inner; i++) cprintf(" ");
    textcolor(LIGHTGRAY);
    textbackground(BLACK);
}

void DlgInputField(int screenRow, int dlgW, const char *label,
                           const char *buf, int cursorVisible) {
    int startX = (SCREEN_W - dlgW) / 2 + 2;
    int inner  = dlgW - 4;
    int lablen = strlen(label);
    int fieldW = inner - lablen - 1;
    ...
    cprintf(" ");
    cprintf("%s", buf);
    for (i = buflen; i < fieldW; i++) cprintf(" ");
    if (cursorVisible && buflen < fieldW) {
        gotoxy(startX + lablen + 1 + buflen, screenRow);
...
int DlgInput(int screenRow, int dlgW, const char *label,
                    char *buf, int maxLen) {
    int len = strlen(buf);
    int ch;
    DlgInputField(screenRow, dlgW, label, buf, 1);
    while (1) {
        ch = getch();
...
void DlgMessage(const char *title, const char *line1,
                        const char *line2, int isError) {
    int w = 50, h = 8;
    int r = DlgDraw(w, h, title);
    if (isError) { textcolor(LIGHTRED); textbackground(BLUE); }
    DlgLine(r,     w, line1);
    if (line2 && line2[0]) DlgLine(r + 1, w, line2);
    DlgLine(r + 3, w, "[ Press any key ]");
    textcolor(LIGHTGRAY); textbackground(BLACK);
    getch();
    DisplayHexEditor();
}
...
int DlgYesNo(const char *title, const char *question) {
    ...
    int r = DlgDraw(w, h, title);
    int sel = 0, ch;
    DlgLine(r, w, question);
    while (1) {
        DlgMenuItem(r + 2, w, "  Yes  ", sel == 0);
        DlgMenuItem(r + 3, w, "  No   ", sel == 1);
        ch = getch();
        if (ch == 0 || ch == 224) ch = getch();
        if (ch == 72 || ch == 75) sel = 0;
        if (ch == 80 || ch == 77) sel = 1;
        if (ch == 'y' || ch == 'Y') { DisplayHexEditor(); return 1; }
        if (ch == 'n' || ch == 'N') { DisplayHexEditor(); return 0; }
        if (ch == 27) { DisplayHexEditor(); return 0; }
        if (ch == 13) { DisplayHexEditor(); return sel == 0; }
    }
}
...
int DlgMenu(const char *title, const char **items, int count) {
    ...
    while (1) {
        for (i = 0; i < count; i++)
            DlgMenuItem(r + i, w, items[i], i == sel);
        ch = getch();
        if (ch == 0 || ch == 224) ch = getch();
        if ((ch == 72 || ch == 75) && sel > 0) sel--;
        if ((ch == 80 || ch == 77) && sel < count - 1) sel++;
        if (ch >= '1' && ch <= '0' + count) {
            DisplayHexEditor(); return ch - '0';
        }
        if (ch == 27) { DisplayHexEditor(); return 0; }
        if (ch == 13) { DisplayHexEditor(); return sel + 1; }
    }
}
...
void DisplayHexEditor(void) {
    ...
    cprintf(" EPROM Editor v1.4 - Mickey W. Lawless [%s MODE]%s", modeStr, Modified ? " *MOD*" : "     ");
    cprintf("%-21s", "");
    ...
    while (1) {
        ...
        if (kbhit()) {
            ...
            if (key == 0 || key == 224) {
                ...
                if (Modified) {
                    if (DlgYesNo(" Exit ", "Buffer modified! Save before exit?"))
                        SaveBufferToFile();
                }
                break;
            } else if (key == 'd' || key == 'D') {
...
/* ========================================================================
   main - v1.4 patched
   Change: removed getch() wait between SendInitString() and
   ReadDataFromCOM1().  The programmer responds to the init string
   immediately and starts streaming data.  The old blocking wait caused
   the 16-byte UART FIFO to overflow so all incoming bytes were lost
   before the read loop ever started.  ReadDataFromCOM1() itself prints
   instructions and waits for the size prompt, which gives the user
   enough time to see what is happening without discarding live data.
   ======================================================================== */
int main(int argc, char *argv[]) {
    ...
    /* Command line filename argument */
    if (argc > 1 && argv[1] != NULL && argv[1][0] != '\0') {
        ...
        textcolor(LIGHTCYAN);
        printf("========================================================================\n");
        printf("               EPROM Data Reader & Hex Editor v1.4\n");
    ...
    if (menuChoice == 1) {
        ...
        /* v1.4 FIX: do NOT getch() here.
           The programmer may already be sending data in response to the
           init string.  Jump straight into ReadDataFromCOM1() which has
           its own prompts and will not miss any incoming bytes. */
        ReadDataFromCOM1();
...
void SendInitString(void) {
    ...
    while (1) {
        ch = getch();
        if (ch == 27) { printf("\n"); return; }
        else if (ch == 13) { initStr[i] = '\0'; break; }
...
void UploadToEPROM(void) {
...

    {
        int w = 54, h = 9, r;
        filename[0] = '\0';
        r = DlgDraw(w, h, " Upload to Programmer ");
        DlgLine(r,     w, "Enter filename to upload:");
        DlgLine(r + 1, w, "(ESC to cancel)");
        if (!DlgInput(r + 3, w, "File:", filename, 12)) {
            DisplayHexEditor(); return;
        }
    ...
    {
        static const char *fmtItems[] = {
            "1. Standard Binary      (raw bytes)",
            "2. BYTEK Binary Code 20 (count+addr+data+sum)",
            "3. DATA I/O Code 10     (arrow header+binary)",
            "4. DEC Binary Code 11   (rubout+null+binary)",
            "5. Motorola S-Record S1 (SREC ASCII)"
        };
        fmt = DlgMenu(" Select Upload Format ", fmtItems, 5);
        if (!fmt) { DisplayHexEditor(); return; }
    }
    ...
    if (fmt == 2) {
        ...
        int w = 54, h = 9, r;
        abuf[0] = '\0';
        r = DlgDraw(w, h, " BYTEK Start Address ");
        DlgLine(r,     w, "Enter start address (hex):");
        DlgLine(r + 1, w, "e.g. 0000");
        DlgInput(r + 3, w, "Addr:", abuf, 6);
        if (abuf[0]) sscanf(abuf, "%lx", &addr);
    }
    ...
    clrscr();
    textcolor(YELLOW);
    printf("Uploading %lu bytes via COM1...\n", byteCount);
    textcolor(LIGHTGRAY);

    switch (fmt) {
        case 1: printf("Format: Standard Binary\n"); break;
        case 2: printf("Format: BYTEK Binary Code 20  Start addr: %06lX\n", addr); break;
        case 3: printf("Format: DATA I/O Binary Code 10\n"); break;
        case 4: printf("Format: DEC Binary Code 11\n"); break;
        case 5: printf("Format: Motorola S-Record S1\n"); break;
    }
    ...
    if (fmt == 1) {
        ...
        while (sent < byteCount) {
            ...
            WriteCOM1(DataBuffer[sent]);
            printf("%02X ", DataBuffer[sent]);
            if (((sent + 1) % 16) == 0) printf("\n");
            sent++;
            delay(2);
        }
        printf("\n");
        ...
        unsigned long sent = 0;
        unsigned int sc = 0;
        unsigned int cnt = (unsigned int)(byteCount & 0xFFFF);

        WriteCOM1((unsigned char)((cnt >> 8) & 0xFF));
        WriteCOM1((unsigned char)(cnt & 0xFF));
        WriteCOM1((unsigned char)((addr >> 16) & 0xFF));
        WriteCOM1((unsigned char)((addr >> 8)  & 0xFF));
        WriteCOM1((unsigned char)(addr & 0xFF));
        delay(10);
        ...
        while (sent < byteCount) {
            ...
            byte = DataBuffer[sent];
            WriteCOM1(byte);
            sc = (unsigned int)((sc + byte) & 0xFFFF);
            printf("%02X ", byte);
            if (((sent + 1) % 16) == 0) printf("\n");
            sent++;
            delay(2);
        ...
        WriteCOM1((unsigned char)((sc >> 8) & 0xFF));
        WriteCOM1((unsigned char)(sc & 0xFF));
        WriteCOM1(0x00);
        WriteCOM1(0x00);
        printf("\nSumcheck: %04X\n", sc);
    }
        ...
        WriteCOM1(0x01); WriteCOM1(0x00); WriteCOM1(0x00);
        WriteCOM1(0x00); WriteCOM1(0x17);
        WriteCOM1(0x00);
        sprintf(hexbuf, "%04X", cnt);
        WriteCOM1((unsigned char)hexbuf[0]);
        WriteCOM1((unsigned char)hexbuf[1]);
        WriteCOM1((unsigned char)hexbuf[2]);
        WriteCOM1((unsigned char)hexbuf[3]);
        WriteCOM1(0xFF);
        delay(10);
        ...
        WriteCOM1(0x00); WriteCOM1(0x00);
        sprintf(hexbuf, "%04X", sc);
        WriteCOM1((unsigned char)hexbuf[0]);
        WriteCOM1((unsigned char)hexbuf[1]);
        WriteCOM1((unsigned char)hexbuf[2]);
        WriteCOM1((unsigned char)hexbuf[3]);
...

        {
            const char *hdr = "HDR";
            unsigned int hcs = 0;
            unsigned int hlen = (unsigned int)(strlen(hdr) + 3);
            hcs = hlen + 0x00 + 0x00;
            for (j = 0; j < (unsigned int)strlen(hdr); j++) hcs += (unsigned char)hdr[j];
            hcs = (~hcs) & 0xFF;
            sprintf(srbuf, "S0%02X0000%s%02X\r\n", hlen, hdr, hcs);
            for (j = 0; srbuf[j]; j++) WriteCOM1((unsigned char)srbuf[j]);
            delay(20);
        }

        while (laddr < byteCount) {
            if (kbhit() && getch() == 27) {
                printf("\nCancelled.\n"); delay(500); return;
            }
            recLen = (byteCount - laddr >= 16) ? 16 : (unsigned int)(byteCount - laddr);
            cs = (recLen + 3);
            cs += (unsigned int)((laddr >> 8) & 0xFF);
            cs += (unsigned int)(laddr & 0xFF);

            slen = sprintf(srbuf, "S1%02X%04lX", recLen + 3, laddr);
            for (j = 0; j < recLen; j++) {
                byte = DataBuffer[laddr + j];
                cs += byte;
                slen += sprintf(srbuf + slen, "%02X", byte);
                printf("%02X ", byte);
            }
            printf("\n");
            cs = (~cs) & 0xFF;
            slen += sprintf(srbuf + slen, "%02X\r\n", cs);
            for (j = 0; j < (unsigned int)slen; j++) WriteCOM1((unsigned char)srbuf[j]);
            laddr += recLen;
            delay(5);
        }
        ...
        {
            char endbuf[16];
            sprintf(endbuf, "S9030000FC\r\n");
            for (j = 0; endbuf[j]; j++) WriteCOM1((unsigned char)endbuf[j]);
        }
        printf("\n");
    }
    ...
    {
        char msg[50];
        sprintf(msg, "Sent %lu bytes successfully.", byteCount);
        DlgMessage(" Upload Complete ", msg, "", 0);
    }
}
...
void LoadFileIntoBuffer(void) {
...

    {
        int w = 54, h = 9, r;
        filename[0] = '\0';
        r = DlgDraw(w, h, " Open File ");
        DlgLine(r,     w, "Enter filename to load:");
        DlgLine(r + 1, w, "(ESC to cancel)");
        if (!DlgInput(r + 3, w, "File:", filename, 12)) {
            BufferSize = 0; DisplayHexEditor(); return;
        }
    }
    if (filename[0] == '\0') { BufferSize = 0; DisplayHexEditor(); return; }

    fp = fopen(filename, "rb");
    if (fp == NULL) {
        DlgMessage(" Error ", "Cannot open file:", filename, 1);
        BufferSize = 0; return;
    }
    ...
    BufferSize = 0;
    {
        int w = 40, h = 6;
        DlgDraw(w, h, " Loading ");
        DlgLine((SCREEN_H - h) / 2 + 3, w, "Please wait...");
    }
    ...
    {
        char msg[50];
        sprintf(msg, "Loaded %lu bytes OK", BufferSize);
        DlgMessage(" Loaded ", msg, filename, 0);
    }
}
...
void DisassembleMenu(void) {
    int sel;
    static const char *cpuItems[] = {
        "1. Motorola 6800",
        "2. Intel 8080",
        "3. Intel 8086"
    };
    sel = DlgMenu(" Disassembler - Select CPU ", cpuItems, 3);
    if (!sel) return;
    CPUType = sel;
    Disassemble();
}
...
/* ======================================================================== */
/* Disassemblers - unchanged from v1.3                                       */
/* ======================================================================== */
