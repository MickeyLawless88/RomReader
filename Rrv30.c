...
void ReadDataFromCOM1(void) {
    ...
    unsigned int value;
    int linePos = 0;
    long lastByteTime;      /* biostime() tick when last byte arrived  */
    long currentTime;
    long waitingMsgTick;    /* for "waiting for data" nag message      */
    int  shownWaiting;

    /* BIOS timer ticks at 18.2 Hz.
       5 seconds of silence = 91 ticks with no new bytes.
       Adjust SILENCE_TICKS if you need more/less timeout.           */
    #define SILENCE_TICKS  91L   /* ~5 seconds                        */
    #define NOWAIT_TICKS   182L  /* ~10 seconds before "waiting" nag  */

    hexStr[0] = 0; hexStr[1] = 0; hexStr[2] = 0;
    BufferSize = 0;

    lastByteTime  = biostime(0, 0L);   /* seed with current tick count */
    waitingMsgTick = lastByteTime;
    shownWaiting   = 0;
    ...
    while (BufferSize < MAX_BUFFER_SIZE) {
        ...
        if (COM1DataReady()) {
            ch = ReadCOM1();
            lastByteTime = biostime(0, 0L);  /* reset real-time clock */
            shownWaiting = 0;
            ...
            /* No byte available - check real elapsed time since last byte */
            currentTime = biostime(0, 0L);

            if (BufferSize > 0 &&
                (currentTime - lastByteTime) >= SILENCE_TICKS) {
                printf("\n");
            ...
            /* Nag message if nothing received yet after ~10 seconds */
            if (BufferSize == 0 &&
                !shownWaiting &&
                (currentTime - waitingMsgTick) >= NOWAIT_TICKS) {
                textcolor(YELLOW);
                printf("Waiting for data from COM1...\n");
                textcolor(LIGHTGRAY);
                waitingMsgTick = currentTime;
            }
        }
    }
