...
void ReadDataFromCOM1(void) {
    ...
    /* BIOS timer ticks at 18.2 Hz.
       10 seconds of silence = 182 ticks with no new bytes.          */
    #define SILENCE_TICKS  182L  /* ~10 seconds of true silence       */
    #define NOWAIT_TICKS   182L  /* ~10 seconds before "waiting" nag  */

    /* idle counter - only checked when no byte is ready, so it only
       accumulates during genuine gaps, not during reception.         */
    {
        long idleCount = 0;
        long lastPrintSize = 0;  /* BufferSize at last screen update  */
        ...
        while (BufferSize < MAX_BUFFER_SIZE) {
            ...
            /* ---- RECEIVE: drain every byte that is ready right now ---- */
            while (COM1DataReady() && BufferSize < MAX_BUFFER_SIZE) {
                ch = inportb(COM1_DATA);   /* direct read - no busy-wait overhead */
                idleCount = 0;             /* byte arrived - reset idle counter   */
                shownWaiting = 0;
            ...
            /* ---- SCREEN UPDATE: only every 64 new bytes ---- */
            if (BufferSize - lastPrintSize >= 64 || 
                (idleCount > 0 && BufferSize > lastPrintSize)) {
                unsigned long i;
            ...
            /* ---- IDLE / TIMEOUT: only when no byte was ready ---- */
            if (!COM1DataReady()) {
                idleCount++;
                /* Small delay only during idle to yield CPU without 
                   blocking the receive path                          */
                if (idleCount % 100 == 0) delay(1);

                if (idleCount > 50000L && BufferSize == 0 && !shownWaiting) {
                    textcolor(YELLOW);
                ...
                /* Declare done after ~10 seconds of genuine silence */
                if (BufferSize > 0 && idleCount > 180000L) {
                    printf("\n");
